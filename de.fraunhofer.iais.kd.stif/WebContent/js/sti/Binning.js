...
Binning.prototype = {
	...
	exactBinning: function(){
		...
		for (var i = 0; i < this.objects.length; i++){
			...
			circleAggregates.push([]);
		   	for (var j = 0; j < this.objects[i].length; j++) {
				...
				if( typeof circleAggregates[i][''+o.longitude][''+o.latitude] == 'undefined' ){
					circleAggregates[i][''+o.longitude][''+o.latitude] = [];
					bins[i].push(circleAggregates[i][''+o.longitude][''+o.latitude]);
				}
				circleAggregates[i][''+o.longitude][''+o.latitude].push(o);
		   	}
	       	}
		...
		for (var i = 0; i < bins.length; i++){
		   	...
		   	for (var j = 0; j < bins[i].length; j++){
		   		var bin = bins[i][j];
			       	var p = new OpenLayers.Geometry.Point(bin[0].longitude, bin[0].latitude, null);
	...
	genericClustering: function(objects,id){
		...
		for (var i = 0; i < this.zoomLevels; i++) {
			...
			clustering.mergeForResolution(resolution);
