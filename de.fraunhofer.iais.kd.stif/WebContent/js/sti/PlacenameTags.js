...
function PlacenameTags(circle,map){
	...
	this.calculatePlacenameTags = function(){
		...
		var allStyles = "", hoverStyle = "", selectedStyle = "", unselectedStyle = "";
		...
		for( var i=0; i<this.placeLabels.length; i++ ){
			...
			var fontSize = 1 + l.weight/this.map.count * STIStatic.maxLabelIncrease;
			if( l.place == "all" ){
				fontSize = 1;
			}
	...
	this.gridLayout = function(){
		...
		var createDiv = function( mod, div ){
			...
			for( var i=0; i<this.placeLabels.length; i++ ){
				if( i%2 == mod ){ 
					height += this.placeLabels[i].div.offsetHeight;
					if( this.placeLabels[i].div.offsetWidth > width ){
						width = this.placeLabels[i].div.offsetWidth;
					}
					if( i>1 ){
						height += 5;
					}
			...
			for( var i=0; i<this.placeLabels.length; i++ ){
				if( i%2 == mod ){ 
					var h = this.placeLabels[i].div.offsetHeight;
					div.appendChild(this.placeLabels[i].div);
					if( mod == 0 ){
						this.placeLabels[i].allStyle += "right: 0px;";
					}
					else {
						this.placeLabels[i].allStyle += "left: 0px;";
					}
					this.placeLabels[i].allStyle += "top: "+height+"px;";
					height += h+5;
					this.placeLabels[i].div.setAttribute('style',this.placeLabels[i].allStyle+""+this.placeLabels[i].unselectedStyle);
				}
			}
