AJS.toInit(function(f){var e=Confluence.storageManager("comments");var a=function(){if(confirm("Wollen Sie den Kommentar wirklich entfernen?")){this.href=this.href+"&confirm=yes";return true}return false};Confluence.Comments={toggleComments:function(g){f("#page-comments").toggleClass("hidden",!g);f(".comments-show-hide").toggleClass("comments-showing",g);f(".icon.comments-show-hide").toggleClass("icon-section-closed",!g);f(".icon.comments-show-hide").toggleClass("icon-section-opened",g);e.setItem("show",g)},bindShowHideToggle:function(){f(".comments-show-hide").click(function(){Confluence.Comments.toggleComments(!f(this).hasClass("comments-showing"));return false})},bindRemoveConfirmation:function(g){f("#comment-"+g+" .comment-action-remove a").click(a)}};if(!f("#comments-section").length){return}if(AJS.isIE6){f(".logo.anonymous").each(function(){var g=document.createElement("div");g.className="replacement";AJS.applyPngFilter(g,this.src);f(this).replaceWith(g)});f(".comment-actions .comment-permalink a").each(function(){f(this).addClass("filtered");var h=f(this).css("background-image").replace(/^url\(\"?|\"?\)$/g,"");var g=h.replace("light","dark");AJS.applyPngFilter(this,h);this.style.cursor="pointer";this.style.background="none";f(this).hover(function(){AJS.applyPngFilter(this,g)},function(){AJS.applyPngFilter(this,h)})})}f("#comments-section .comment:odd").addClass("odd");f(".comment-action-remove a").click(a);if(AJS.Meta.getBoolean("show-comments")){Confluence.Comments.toggleComments(true)}else{var c=e.getItem("show");if(c!=null){Confluence.Comments.toggleComments(c=="true")}}Confluence.Comments.bindShowHideToggle();var d=f("#addcomment.comment-text"),b=f("#comments-text-editor textarea");b.focus(function(){d.addClass("active")}).blur(function(){if(!f.trim(b.val()).length){d.removeClass("active")}}).bind("reset.default-text",function(){d.removeClass("active")});f("form[name='textcommentform']").submit(function(){var g=b.val();if(!f.trim(g)){alert("Kommentar enth\u00E4lt keinen Text. Leere Kommentare k\u00F6nnen nicht erstellt werden.");return false}return true});f("#add-comment-rte").click(function(){if(!b.hasClass("placeholded")){e.setItem("text-comment",f.trim(b.val()))}});if(f("#addcomment #rte").length){AJS.bind("init.rte",function(i,h){var g=e.getItem("text-comment");if(g){h.editor.setContent(g);e.setItem("text-comment","")}})}});
(function(D){function B(J){var E,K;
if(J.offsetX===undefined){var I=0,H=0,G=J.target,F;
do{if(G.scrollTop!=0||G.scrollLeft!=0){F=G
}I+=G.offsetLeft;
H+=G.offsetTop;
G=G.offsetParent
}while(G&&G!=G.offsetParent);
E=J.pageX+(F?F.scrollLeft:0)-I;
K=J.pageY+(F?F.scrollTop:0)-H
}else{E=J.offsetX;
K=J.offsetY
}return E>=3&&E<=14&&K>=3&&K<=14
}function A(E){return E.currentTarget===E.target
}var C;
D(document).delegate("ul.inline-task-list > li[data-inline-task-id]",{click:function(I){if(A(I)&&B(I)){var H=D(I.target).toggleClass("checked"),E=H.hasClass("checked")?"CHECKED":"UNCHECKED",G=H.data("inline-task-id"),J=H.closest("ul").attr("data-inline-tasks-content-id")||AJS.params.pageId,F=AJS.contextPath()+"/rest/inlinetasks/1/task/"+J+"/"+G+"/";
D.ajax({type:"POST",url:F,data:{status:E,trigger:"VIEW_PAGE"},dataType:"text",timeout:30000,error:function(L,M,K){AJS.log("Inline Task #"+G+" was not persisted to "+E+" because of "+K+" (status: "+M+")");
H.toggleClass("checked");
C=C||Confluence.InlineTasks.Notice();
C.show()
}})
}},mousemove:function(E){if(A(E)){if(B(E)){D(E.target).addClass("hover")
}else{D(E.target).removeClass("hover")
}}},mouseout:function(E){if(A(E)){D(E.target).removeClass("hover")
}},mousedown:function(E){if(A(E)&&B(E)){D(E.target).addClass("active")
}},mouseup:function(E){if(A(E)&&B(E)){D(E.target).removeClass("active")
}}})
})(AJS.$);
...
Confluence.InlineTasks.Templates.notice = function(opt_data, opt_sb) {
  ...
  output.append('<div class="aui-message error" id="inline-tasks-notice">', "Ihre Aufgabe konnte nicht gespeichert werden. \x3cbr/\x3eDaf\xfcr kommen mehrere Gr\xfcnde in Frage.", '&nbsp;&nbsp;<a href="', soy.$$escapeHtml("http://docs.atlassian.com/confluence/docs-43/Failed+Update+to+Task+List"), '" class="notice-learn-more">', soy.$$escapeHtml("Erfahre mehr"), '</a>&nbsp;&nbsp;<a href="#" class="notice-close">', soy.$$escapeHtml("Akzeptieren"), '</a></div>');
        ...
        AJS.I18n.get("com.atlassian.confluence.plugins.confluence-like", function () {
            ...
            function updateLikeSummary(data, contentId, $summaryContainer) {
                ...
                $summaryContainer.find(".likes").click(function () {
                    ...
                    likesDialog.addHeader("Personen, denen das gef\u00E4llt");
                    likesDialog.addPanel("Panel 1", "<div class='spinner-container'></div>");
                    likesDialog.addCancel("Schlie\u00DFen", function (dialog) {
                        dialog.remove();
                        likesDialog = undefined;
                    });
            ...
            function createHandlerFor(type, contentId, contentType) {
                ...
                return function () {
                    ...
                    $likeLink
                        ...
                        .html(type === LIKE ? "Gef\u00E4llt mir nicht" : "Gef\u00E4llt mir");
            ...
            $.ajax({
                ...
                if (AJS.Meta.get("remote-user")) {
                    ...
                    $contentLikeSection.find(".like-button")
                        ...
                        .html(existingLike ? "Gef\u00E4llt mir nicht" : "Gef\u00E4llt mir");
                }
...
Confluence.Templates.Likes.likeSection = function(opt_data, opt_sb) {
  var output = opt_sb || new soy.StringBuilder();
  output.append('<div id="likes-section">', (opt_data.showLikeButton) ? '<a href="" class="like-button">' + soy.$$escapeHtml("Gef\xe4llt mir") + '</a>' : '', '<span class="like-summary"></span></div>');
  return opt_sb ? '' : output.toString();
};
...
Confluence.Templates.Likes.commentLikeSection = function(opt_data, opt_sb) {
  var output = opt_sb || new soy.StringBuilder();
  output.append('<li class="comment-action-like">', (opt_data.showLikeButton) ? '<a href="" class="like-button">' + soy.$$escapeHtml("Gef\xe4llt mir") + '</a>' : '', '<span class="like-summary"></span></li>');
  return opt_sb ? '' : output.toString();
};
...
Confluence.Templates.Likes.likesDialog = function(opt_data, opt_sb) {
  ...
  for (var likeIndex20 = 0; likeIndex20 < likeListLen20; likeIndex20++) {
    ...
    output.append('<li><div class="avatar-container"><a href="', soy.$$escapeHtml(likeData20.user.url), '"><img class="like-user-avatar" src="', soy.$$escapeHtml(likeData20.user.avatarUrl), '"></a></div><div class="like-user"><a class="like-user-link" href="', soy.$$escapeHtml(likeData20.user.url), '">', soy.$$escapeHtml(likeData20.user.fullName), '</a></div><div class="follow-button-container aui-toolbar"><ul class="toolbar-group"><li class="toolbar-item">', (likeData20.user.followedByRemoteUser) ? soy.$$escapeHtml("Kontakt") : (opt_data.remoteUser && opt_data.remoteUser.length > 0) ? '<button data-username="' + soy.$$escapeHtml(likeData20.user.name) + '" class="likes-dialog-follow-button toolbar-trigger">' + soy.$$escapeHtml("Folgen") + '</button>' : '', '</li></ul></div></li>');
  }
...
AJS.toInit(function(A){Confluence.KeyboardShortcuts&&Confluence.KeyboardShortcuts.Autoformat.push({action:"@",context:"autoformat.autocomplete",description:"Erw\u00E4hnung"})
});
(function($){
    ...
    Confluence.SortableTables = {
        init: function(){
            ...
            tables = tables.filter(function(){
                ...
                //all cells in first row are th elements and none have a class of nohighlight
                return firstRowCells.filter('th').length == firstRowCells.length && !firstRowCells.hasClass('nohighlight');
            });
        },
        enable: function(){
            ...
            tables.tablesorter({
                ...
                "textExtraction" : function(node){
                    ...
                    return $(node).text().replace( trimLeft, "" ).replace( trimRight, "" );
                },
...
if(Confluence.KeyboardShortcuts){Confluence.KeyboardShortcuts.Editor.push({context:"editor.actions",descKey:"JIRA-Vorgang einf\u00FCgen"+":",keys:[["Ctrl+Shift+J"]]})
};
