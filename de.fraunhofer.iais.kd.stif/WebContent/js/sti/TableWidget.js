 ...
 * @constructor
 */
function TableWidget(core,containerDiv){
	...
	initToolbar: function(){
		...
		navigation.appendChild(selectors);
 		...
 		if( STIStatic.tableSelectPage ){
			...
			this.selectPage.title = STIStatic.getString(language,'selectTablePageItemsHelp');
			...
			this.selectPage.onclick = function(){
				...
				if( selectPageItems ){
					...
					table.selectPage.title = STIStatic.getString(language,'selectTablePageItemsHelp');
				}
				else {
					var items = 0;
					for( var i=table.first; i<table.elements.length; i++ ){
					...
					table.selectPage.title = STIStatic.getString(language,'deselectTablePageItemsHelp');
				}
				table.update();
...
//		selectors.style.width = (this.filter.offsetWidth + this.selectAll.offsetWidth + this.selectPage.offsetWidth)+"px";
		...
		var dropdown = new Dropdown(this.resultsDropdown,this.toolbar,itemNumbers,'selectMapType',0,0);
		for( var i=0; i<STIStatic.validResultsPerPage.length; i++ ){
	...
	setPagesText: function(){
		var infoText = STIStatic.getString('en','pageInfo');
	...
	setResultsText: function(){
		if( this.elements.length == 0 ){
			...
			var infoText = STIStatic.getString('en','resultsInfo');
